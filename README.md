...
Основной скрипт run.php содержить итератор и callback-функцию, которая в консольно режиме вызывает для каждой CMS скрипты по пути /\<CMS\>/updater.php. В первом параметре скрипта передаётся html_query, содержащая параметры для работы, в частности $path - путь к текущей диретории. Чтобы извлечь параметры можно использовать например ```parse_str($argv[1],$_REQUEST);```
...
В целом содержание /\<CMS\>/updater.php произвольное. Для текущих CMS EVO и REVO процесс разбит на шаги, кторые указаны в /\<CMS\>/config.inc.php. Каждый шаг вызывает скрипт /\<CMS\>/steps/\<stepname\>.php

## db_pass_change - массовое изменение паролей к БД
Пробегает по сайтам и обновляет пароли к базам данных.

Основной скрипт run.php cодежрдит функцию **changeDBPass** осушществляющую смену пароля и **$CMSChangePass** - callback для CMSIterator, который изменяет пароль через **changeDBPass** и пытается записать новый пароль в конфигурацию CMS, а в случае неудачи меняет пароль обртано.
