...
2. Создать в папке .custom файл config.inc.php с настройками специфичными для аккаунта (пример можно взять на effect43)
...
4. Создаём в панели в пункте 'CronTab' задачу вида:
```/usr/local/php-cgi/7.1/bin/php ~/.custom/scripts/\<script_dir\>/run.php```
5. Запуск задачи настраиваем по потребностям например раз в месяц, либо запускаем вручную по кнопке вида play справа от задачи
6. Пароли обновляются только у баз данных, поэтому для входа в phpmyadmin из панели нужно будет вводить пароль

# Обновление репозиториев:

1. Внести изменения восновном репозитрии (effect43)
2. При необходимости в файле .custom/git.autopull.after.php раскомментировать команды для обновления CMS или смены паролей
3. Обновить репозиторий github через консоль или выполнив autocommit.php. В качестве параметра можно передать комментарий к изменениям.
```/usr/local/bin/php7.1 ~/.git.autocommit.php "Комменатрий"```
...
Сдесь доступны переменные $accaunt и $site, которые позволяют задать разны е настройки для разных аккаунтов или сайтов
