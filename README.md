...
1. Внести изменения в основном репозитрии (effect43)
2. При необходимости в файле .custom/git.autopull.after.php раскомментировать команды для обновления CMS или смены паролей
3. Обновить репозиторий github через консоль или выполнив .custom/git.autocommit.php. В качестве параметра можно передать комментарий к изменениям.
```/usr/local/bin/php7.1 ~/.custom/git.autocommit.php "Комменатрий"```
...
- Скрипты обработки располагаются в scripts:
    ...
    - [db_phpadmin_fix](#db_phpadmin_fix---задаёт-пароли-к-базам-данных-из-конфигов-через-api-beget) - задаёт пароли к базам данных из конфигов через api Beget
...
config.acc.inc.php используется для хранения настроек общих для обновления всех CMS, но специфичных для аккаунта. этот файл не входит в репозиторий.
...
###Обновление, создание и удаление пользователей:
- Если нужно массово удалить пользователя это делается через указание его в конфиге ```scripts/batch_changes/config.inc.php``` в массиве **$config['users']['remove']**
- Если нужно массово обновить пользователя это делается через указание его в конфиге ```scripts/batch_changes/config.inc.php``` в массиве **$config['users']['update']**.  
В качестве ключа используется username или email пользователя, в качестве занчения массив полей и значений, которые нужно изменить
- Если нужно массово создать пользователя это делается через указание его в конфиге ```scripts/batch_changes/config.inc.php``` в массиве **$config['users']['create']**.  
В качестве ключа используется username пользователя, в качестве занчения массив полей пользователя.  
Для указания значений полей профиля используйте ключ **profile**.  
Для указания групп используйте ключ **groups** - массив с ключами-названиями групп и значениями - id ролей ( 1 для Member, 2 для Super User)

> Если нужно массово сменить пароль пользователю или создать нового(что требует указания парроля), это можно сделать через соотвествующий конфиг, но не рекомендуется, так как изменения будут залиты на github.  
> Если есть острая необходимость, то так можно сделать, но через **10 минут** нужно будет откатить изменения к предполседнему коммиту   
> Для изменения паролей лучше использовать конфиг специфичный для сервера ```scripts/batch_changes/config.acc.inc.php```.  
> Этот файл не является частью репозитория, но подключается при обработке, что позволяет хранить в нём данные, которые не должны быть публичными.  
> Пример этого файла можно взять на основном аккаунте, структура этого конфига такая же как и у ```scripts/batch_changes/config.inc.php```  
> config.acc.inc.php не перезаписывает config.inc.php, а рекурсивно соединяется, это значит что могут быть указаны как глобальные пользователи для удаления/обновления так и специфичные для этого аккаунта, в итоговом конфиге будут пользователи из обоих конфигов
...
Пароли обновляются только у баз данных, поэтому для входа в phpmyadmin из панели нужно будет вводить пароль

## db_phpadmin_fix - задаёт пароли к базам данных из конфигов через api Beget
Использовать, когда слетают пароли при переносе на другой сервер  
Для работы нужно обязательно передать логин и пароль к аккаунту в параметрах вызова:
```/usr/local/php-cgi/7.1/bin/php ~/.custom/scripts/db_phpadmin_fix/run.php accaunt pwd```
