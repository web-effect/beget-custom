...
### Обновление и удаление пользователей:
 ...
 - Если нужно массово сменить пароль пользователю, это тоже можно сделать через данный конфиг в $config['users']['update'], но не рекомендуется, так как изменения будут залиты на github. Если есть острая необходимость, то так можно сделать, но через 10 минут нужно будет удалить записи с паролем из конфига и залить изменения снова
- Для изменения паролей лучше использовать конфиг специфичный для сервера scripts/batch_changes/config.acc.inc.php. Этот файл не является частью репозитория, но подключается при обработке, что позволяет хранить в нём данные, которые не должны быть публичными. Пример этого файла можно взять на основном аккаунте, структура этого конфига такая же как и у scripts/batch_changes/config.inc.php
